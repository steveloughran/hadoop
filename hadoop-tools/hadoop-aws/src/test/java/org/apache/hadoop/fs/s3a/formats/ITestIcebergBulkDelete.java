...
package org.apache.hadoop.fs.s3a.formats;
 ...
 * <p>
 * Chooses a very small page size then measures invocation statistics to make sure there were multiple
 * pages. It also uses a hasPathCapabilities() probe.
 */
@ParameterizedClass
@MethodSource("params")
public class ITestIcebergBulkDelete extends AbstractS3ACostTest {
  ...
  public Configuration createConfiguration() {
    ...
    conf.setBoolean(BULK_DELETE_ENABLED, true);
  ...
  @Test
  public void testBulkDeleteEnabledInIceberg() throws Throwable {
    Assertions.assertThat(fileIO)
        .describedAs("Iceberg file IO %s", fileIO)
        .isInstanceOf(PathCapabilities.class);
    var pc = (PathCapabilities) fileIO;
    final var path = methodPath();
    Assertions.assertThat(pc.hasPathCapability(path, BULK_DELETE_ENABLED))
        .describedAs("bulk delete enabled in %s for %s", pc, path)
        .isTrue();
  }
  ...
  public static List<String> stringList(List<Path> files) {
    return files.stream().map(p -> toString(p)).toList();
  }
  ...
  public static List<String> stringList(Path path) {
    return newArrayList(toString(path));
  }
  ...
  private List<FileInfo> listPaths(Path path) throws Throwable {
    return newArrayList(fileIO.listPrefix(toString(path)));
  }
